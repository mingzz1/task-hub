/* MeetingsView — schedule + agenda (물어볼 것) + 1:1 notes. Exposes window.MeetingsView */
(function () {
  const { Avatar, fmtDue, WD, cadenceLabel, nextMeetingDate, KIND_LABEL } = window.UI;
  const Icons = window.Icons;
  const { useState } = React;

  // when switching kind, seed sensible defaults for the new kind
  function kindDefaults(k, m) {
    if (k === "recurring") return { kind: k, weekday: m.weekday != null ? m.weekday : 1, time: m.time || "10:00", everyWeeks: m.everyWeeks || 1, anchor: window.AppData.today };
    if (k === "oneoff") return { kind: k, date: m.date || window.AppData.today, time: m.time || "10:00" };
    if (k === "series") return { kind: k, sessions: (m.sessions && m.sessions.length) ? m.sessions : [{ id: window.AppData.uid(), date: window.AppData.today, time: m.time || "10:00", label: "1차" }] };
    return { kind: k };
  }

  function MeetingsView({ state, activeId, setActiveId, onAddMeeting, onUpdateMeeting, onDeleteMeeting,
      ...
      React.createElement("div", { className: "meet-main" },
        ...
        editSched ? React.createElement("div", { className: "sched-edit" },
          ...
          (meeting.kind || "recurring") === "recurring" ? React.createElement("div", { className: "sched-fields" },
            ...
            React.createElement("select", { className: "selectbox", style: { width: "auto" }, value: meeting.weekday, onChange: (e) => onUpdateMeeting(meeting.id, { weekday: +e.target.value, anchor: window.AppData.today }) },
        ...
        !editSched && meeting.kind === "series" && (meeting.sessions || []).length ? React.createElement("div", { className: "session-list" },
          meeting.sessions.slice().sort((a, b) => (a.date || "").localeCompare(b.date || "")).map((s) => React.createElement("div", { className: "session-chip" + (window.UI.dayDiff(s.date) < 0 ? " past" : ""), key: s.id },
