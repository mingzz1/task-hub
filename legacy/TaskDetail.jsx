/* TaskDetail — slide-in editor sheet. Exposes window.TaskDetail */
(function () {
  const { COLUMNS, PEOPLE } = window.AppData;
  const { DueBadge, Avatar } = window.UI;
  ...
  function TaskDetail({ task, state, onUpdate, onDelete, onClose, onOpenBookmark }) {
    const [bmPicker, setBmPicker] = useState(false);
    const [peopleInput, setPeopleInput] = useState("");
    const [newCheck, setNewCheck] = useState("");

    const project = state.projects.find((p) => p.id === task.project);
    const meeting = state.meetings.find((m) => m.id === task.meetingId);
    ...
    const addCheck = () => {
      ...
      if (!t) return;
      setChecklist([...checklist, { id: window.AppData.uid(), text: t, done: false }]);
      setNewCheck("");
    };
    const toggleBm = (id) => {
      ...
      React.createElement("div", { className: "sheet", onMouseDown: (e) => e.stopPropagation() },
        ...
        React.createElement("div", { className: "sheet-head" },
          React.createElement("div", { className: "chips", style: { flex: 1 } },
            COLUMNS.map((c) => React.createElement("button", {
              ...
              onClick: () => onUpdate({ status: c.id, ...(c.id === "done" && !task.completedAt ? { completedAt: window.AppData.today, quarter: state.quarter } : {}) }),
            }, c.label))),
        ...
        React.createElement("div", { className: "sheet-foot" },
          ...
          task.status === "done"
            ...
            : React.createElement("button", { className: "btn btn-primary", onClick: () => onUpdate({ status: "done", completedAt: task.completedAt || window.AppData.today, quarter: task.quarter || state.quarter }) },
